...
def offtime_computing():
    ...
    while True:
        if offtime:
            alphabeta(offtime_pos, offtime_depth, -WIN_SCORE, WIN_SCORE, False, lambda: not offtime)
            offtime_depth += 1
...
def move(board, col, **kwargs):
    ...
    while get_cur_time() < move_stop_time and depth < MAX_DEPTH:
        ...
        #childrens[head].sort(key=lambda c: scores[c] if c not in mmx_scores else 100*mmx_scores[c], reverse=True)
    ...
    if kwargs.get('time', False):
        print("base:", "d:", depth-1, "\tt:", get_cur_time(), len(scores), len(childrens))
